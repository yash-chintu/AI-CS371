...
int minimax(char b[3][3], bool isMax, int alpha, int beta)
{
        ...
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                if (b[i][j] == '-')
                {
                    b[i][j] = 'X';
                    cnt++;
                    val = max(val, minimax(b, !isMax, alpha, beta));
                    alpha = max(alpha, val);
                    b[i][j] = '-';
                    if (beta <= alpha)
                        return val;
        ...
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                if (b[i][j] == '-')
                {
                    b[i][j] = 'O';
                    cnt++;
                    val = min(val, minimax(b, !isMax, alpha, beta));
                    beta = min(beta, val);
                    b[i][j] = '-';
                    if (beta <= alpha)
                        return val;
    ...
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            if (board[i][j] == '-')
            {
                board[i][j] = 'X';
                cnt++;
                int temp = minimax(board, false, INT_MIN, INT_MAX);
                board[i][j] = '-';
                if (temp > val)
                {
